...
class ProductListPageEnfant extends React.Component {
    constructor(props) {
        super(props)
            ...
            .then(response => {
                if (response.data) {
                    const filteredArray = response.data.filter(oneProduct => {
                        if (oneProduct.Genre.includes("Enfant")) {
                            return true;
                        }
                        else {
                            return false;
                ...
                {isDataReceived ?
                    (
                        <section className="product-list-page">
                            {headBannerData[0] ?
                                <div style={{ backgroundImage: `url(${headBannerData[0].data.hero_image.url})` }} className="head-banner lunettes-genre-head-banner">
                                    <h2>Lunettes</h2>
                                    <h1>{headBannerData[0].data.headline[0].text}</h1>
                                </div>
                                :
...
export default ProductListPageEnfant;