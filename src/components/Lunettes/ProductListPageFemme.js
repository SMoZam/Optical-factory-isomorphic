...
class ProductListPageFemme extends React.Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount() {
            ...
            .then(response => {
                if (response.data) {
                    const filteredArray = response.data.filter(oneProduct => {
                        if (oneProduct.Genre.includes("Femme") || oneProduct.Genre.includes("Mixte")) {
                            return true;
                        }
                        else {
                            return false;
        ...
        api.get("/women")
            .then(response => {
                ...
                const metaTitle = response.data[1].data.title[0].text;
                const metaDescription = response.data[1].data.meta_description[0].text;
                const FacebookData = response.data[1].data.body;
                ...
                {isDataReceived ?
                    (
                        <section className="product-list-page">
                            {headBannerData[0] ?
                                <div style={{ backgroundImage: `url(${headBannerData[1].data.hero_image.url})` }} className="head-banner lunettes-genre-head-banner">
                                    <h2>Lunettes</h2>
                                    <h1>{headBannerData[1].data.headline[0].text}</h1>
                                </div>
                                :
...
export default ProductListPageFemme;